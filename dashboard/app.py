...
with tab1:
    try:
        ...
        # Slice for the failure class (class 1) and single input sample
        if len(shap_values.shape) == 3:
            single_explanation = shap_values[0, :, 1]
        else:
            single_explanation = shap_values[0]

        # Force Matplotlib text colors to white for dark theme readability
        plt.rcParams.update({
            'text.color': '#FFFFFF',
        ...
        # Override text color of all labels generated by SHAP inside the figure
